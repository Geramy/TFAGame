...
using TFAGame.Network;

namespace TFAGame
{
    public delegate void ModuleCallback(ref PacketReader packet_reader, PacketType p_type, ref ConnectionManager cm);
    public delegate PacketHandler ModuleAddonCallback(PacketType p_type, string CustomPacketType, ref PacketReader packet_reader, PacketHandler p, ref ConnectionManager cm);
    public static class ModuleHandler
    {
        internal static Dictionary<string, GraphicModule> GraphicsModules = new Dictionary<string, GraphicModule>();
            ...
            if (CoreModulesStarted == false)
            {
                ...
                for (; i < ModuleHandler._CoreModules.Count(); i++)
                {
                    ...
                    if (ModuleHandler._CoreModules[key].started == false)
                                                if(Config.Configuration.ContainsKey(key) && (bool)Config.Configuration[key] == true)
                                InvokeStartCoreModule(ModuleHandler._CoreModules[key]);
                                                else if(!Config.Configuration.ContainsKey(key))
                                                        InvokeStartCoreModule(ModuleHandler._CoreModules[key]);
                }
            }
            else
